...
module.exports = async (alphax, m, commands, chatUpdate) => {
    try {
        ...
        let metadata = isGroup ? await alphax.groupMetadata(from) : {}
        ...
        let isBotAdmin = isGroup ? groupAdmin.includes(alphax.user?.jid) : false
        let isAdmin = isGroup ? groupAdmin.includes(sender) : false
        let isOwner = [alphax.user?.jid, ...config.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(sender)
        ...
        user.expiredCheck(alphax, m, _user);

        if (config.options.autoRead) (alphax.type == "legacy") ? await alphax.chatRead(m.key, 1) : await alphax.sendReadReceipt(from, sender, [m.id])
        ...
        if (isMedia && m.msg.fileSha256 && (m.msg.fileSha256.toString("hex") in global.db.sticker)) {
            ...
            let messages = await generateWAMessage(m.from, { text, mentions }, {
                userJid: alphax.user.jid,
                quoted: m.quoted && m.quoted.fakeObj
            })
            messages.key.fromMe = await areJidsSameUser(m.sender, alphax.user.jid)
            ...
            alphax.ev.emit("messages.upsert", msg)
        }
        ...
        if (isGroup && isBotAdmin && isAntilink && !isAdmin && !isOwner) {
            if (budy.match("://chat.whatsapp.com/")) {
                setTimeout( () => {
                    alphax.groupParticipantsUpdate(from, [sender], "remove")
                }, 5 * 1000)
                setTimeout( () => {
        ...
        if (isAntidelete && m.message && m.message.protocolMessage && m.message.protocolMessage.type == 0) {
            ...
            let msg = await alphax.serializeM(await Store.loadMessage(key.remoteJid, key.id))
            ...
            alphax.relayMessage(m.from, msg.message, { messageId: msg.id })
            await alphax.sendText(m.from, tekss, msg, { mentions: [msg.sender] })
        }
