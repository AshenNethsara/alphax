...
module.exports = {
	...
	noLimit: true,
    start: async(alphax, m, {isPremium, isOwner}) => {
		let statuses
		try {
			statuses = await alphax.fetchStatus(m.sender);
		} catch {
			statuses = "Nothing.."
		}
		...
		try {
			var pp = await alphax.profilePictureUrl(m.sender, "image");
		} catch {
		...
		caption += `│\n`
		caption += `└───────⭓\n`
		alphax.sendFile(m.from, pp, "", m, { caption })
    }
}