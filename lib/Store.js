...
function bind(alphax) {
    ...
    alphax.ev.on('connection.update', update => {
        Object.assign(state, update)
    })

    alphax.ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        /**if (isLatest) {
            chats.clear()
        }**/
    ...
    alphax.ev.on('contacts.set', ({ contacts: newContacts }) => {
    ...
    alphax.ev.on('messages.set', ({ messages: newMessages, isLatest }) => {
        if (isLatest) {
   ...
   alphax.ev.on('contacts.update', updates => {
        for (const update of updates) {
            Object.assign(contacts[update.id], update)
        }
    })

    alphax.ev.on('chats.upsert', newChats => {
        chats.upsert(...newChats)
    })

    alphax.ev.on('messages.delete', item => {
        if ('all' in item) {
    ...
    alphax.ev.on('chats.delete', deletions => {
        for (const item of deletions) {
            chats.deleteById(item)
        }
    })

    alphax.ev.on('chats.update', updates => {
    ...
    alphax.ev.on('presence.update', ({ id, presences: update }) => {
        presences[id] = presences[id] || {}
        Object.assign(presences[id], update)
    })

    alphax.ev.on('messages.upsert', ({ messages: newMessages, type }) => {
        switch (type) {
            case 'append':
            case 'notify':
                for (const msg of newMessages) {
    ...
    /**alphax.ev.on('messages.update', updates => {
    ...
    alphax.ev.on('groups.update', updates => {
        for (const update of updates) {
    ...
    alphax.ev.on('group-participants.update', ({ id, participants, action }) => {
    ...
    alphax.ev.on('message-receipt.update', updates => {
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            const retrieve = async (count, cursor) => {
                const result = await (alphax === null || alphax === void 0 ? void 0 : alphax.fetchMessagesFromWA(jid, count, cursor))
                return result || []
            }
        ...
        loadMessage: async (jid, id) => {
            ...
            if (!message) {
                message = await (alphax === null || alphax === void 0 ? void 0 : alphax.loadMessageFromWA(jid, id))
            }
            return message
        },
        mostRecentMessage: async (jid) => {
            ...
            if (!message) {
                const [result] = await (alphax === null || alphax === void 0 ? void 0 : alphax.fetchMessagesFromWA(jid, 1, undefined))
                message = result
            }
            return message
        },
        fetchImageUrl: async (jid) => {
            ...
            if (!contact) {
                return alphax === null || alphax === void 0 ? void 0 : alphax.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (alphax === null || alphax === void 0 ? void 0 : alphax.profilePictureUrl(jid))
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (alphax === null || alphax === void 0 ? void 0 : alphax.groupMetadata(jid))
            }
            return groupMetadata[jid]
        },
        fetchBroadcastListInfo: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (alphax === null || alphax === void 0 ? void 0 : alphax.getBroadcastListInfo(jid))
            }
            return groupMetadata[jid]
        },
        fetchMessageReceipts: async ({ remoteJid, id }) => {
            ...
            if (!receipts) {
                receipts = await (alphax === null || alphax === void 0 ? void 0 : alphax.messageInfo(remoteJid, id))
                if (msg) {
